...
export const metadata = {
  ...
  description: `Hela Rusta och matcha-marknaden som konstellation: alla avtals viktade resultat över tid. Lyft upp till ${MAX_COMPARE} avtal och jämför.`,
};
...
export default async function ComparePage({ searchParams }: Props) {
  ...
  if (keys) {
    ...
    const valid = new Set(cloud.map((s) => contractKey(s.supplier, s.delivery_area)));
    initialKeys = requested.filter((k) => valid.has(k));
  } else {
    const top5 = latest ? await getTopContracts(latest, 5) : [];
    initialKeys = top5.map((r) => contractKey(r.supplier, r.delivery_area));
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--text-dim)] mt-1 max-w-2xl">
          ...
          eller sök för att lyfta fler (max {MAX_COMPARE} för läsbarhet). Vi slår aldrig ihop områden till beräknade snitt.
        </p>
